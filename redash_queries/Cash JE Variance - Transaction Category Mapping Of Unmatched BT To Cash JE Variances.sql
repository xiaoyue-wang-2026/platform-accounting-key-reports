...
--The below query identifies the payment records that makes up the unmatched BTs.
get_unmatched_bts AS(
SELECT
...
WHERE
    pr.bank_transaction_ids IN (
        SELECT REPLACE(bank_transaction_id, ',', '')
